...
import { getPublicSparePartBySku, getPublicSpareParts, type SparePart } from '@/lib/api/spare-parts';

export default function SparePartDetailClient() {
  ...
  useEffect(() => {
    if (!slug) return;
    getPublicSparePartBySku(slug).then(async ({ data }) => {
      if (!data) { setLoading(false); return; }
      setPart(data);
      const { data: rel } = await getPublicSpareParts({ category: data.category });
      ...
      <div className="max-w-screen-xl mx-auto px-6 lg:px-10 py-8">
        <div className="grid lg:grid-cols-2 gap-12">
          <div className="aspect-[4/3] overflow-hidden bg-zinc-50">
            {part.image && <img src={part.image} alt={part.name} className="w-full h-full object-cover" style={{ objectPosition: 'center 30%' }} />}
          </div>

          <div>
            <p className="text-xs tracking-widest uppercase text-zinc-400 mb-2">{part.category}</p>
            ...
            <div className="bg-zinc-50 p-5 mb-6 space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-zinc-900">{part.category}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-zinc-500">Availability</span>
                <span className={`font-medium ${part.stock_qty > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                  {part.stock_qty > 0 ? `In Stock (${part.stock_qty})` : 'Out of Stock'}
                </span>
              </div>
            </div>
        ...
        {related.length > 0 && (
          <div className="mt-16 border-t border-zinc-100 pt-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              {related.map((item) => (
                <Link key={item.id} href={`/spare-parts/${item.sku.toLowerCase()}`} className="group border border-zinc-100 hover:border-zinc-300 transition-colors p-4">
                  <div className="aspect-[4/3] overflow-hidden bg-zinc-50 mb-3">
                    {item.image && <img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" style={{ objectPosition: 'center 30%' }} />}
                  </div>
