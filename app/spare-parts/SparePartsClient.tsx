...
import { getPublicSpareParts, getSparePartCategories, type SparePart } from '@/lib/api/spare-parts';

export default function SparePartsClient() {
  const [parts, setParts] = useState<SparePart[]>([]);
  const [partCategories, setPartCategories] = useState<string[]>(['All']);
  ...
  const load = useCallback(async () => {
    ...
    const [{ data }, cats] = await Promise.all([
      getPublicSpareParts({ search, category }),
      ...
      <section className="py-10 bg-white">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
          <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6">
            ...
            <p className="text-sm text-zinc-400 md:ml-auto">{loading ? '...' : `${parts.length} parts`}</p>
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="animate-pulse border border-zinc-100 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {parts.map((part) => (
                <Link key={part.id} href={`/spare-parts/${part.sku.toLowerCase()}`} className="group border border-zinc-100 hover:border-zinc-300 transition-colors p-4">
                  <div className="aspect-[4/3] overflow-hidden bg-zinc-50 mb-4">
                    {part.image && (
                      <img src={part.image} alt={part.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" style={{ objectPosition: 'center 30%' }} />
                    )}
                  </div>
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className="text-xs text-zinc-400">{part.category}</p>
