...
export default function CareersPage() {
  ...
  const [appForm, setAppForm] = useState<AppForm>({ name: '', email: '', phone: '', linkedin: '', coverLetter: '', resumeName: '' });
  ...
  const openApply = (job: Career) => {
    setApplyJob(job);
    setAppForm({ name: '', email: '', phone: '', linkedin: '', coverLetter: '', resumeName: '' });
    setAppErrors({});
                ...
                {selectedJob?.id === job.id && (
                  <div className="border-t border-zinc-100 p-5 md:p-6 bg-zinc-50">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        <h4 className="label-field mb-3">Responsibilities</h4>
              ...
              {appSubmitted ? (
                ...
                <form onSubmit={handleApplySubmit} className="space-y-4" noValidate>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="label-field" htmlFor="app-linkedin">LinkedIn URL</label>
                      <input id="app-linkedin" type="url" value={appForm.linkedin} onChange={setField('linkedin')}
                        placeholder="linkedin.com/in/..." className="input-field" />
                    </div>
                  </div>
                    ...
                    <label className={`flex flex-col items-center justify-center p-6 border-2 border-dashed cursor-pointer transition-colors hover:border-zinc-500 ${appErrors.resumeName ? 'border-red-300' : appForm.resumeName ? 'border-zinc-400 bg-zinc-50' : 'border-zinc-200'}`}>
                      <input type="file" accept=".pdf,.doc,.docx" className="hidden"
                        onChange={(e) => {
                          ...
                          if (file) setAppForm((f) => ({ ...f, resumeName: file.name }));
                        }} />
                      {appForm.resumeName ? (
                  ...
                  <button type="submit" disabled={appLoading}
                    ...
                    {appLoading ? 'Submitting...' : (<>Submit Application <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" /></>)}
                  </button>
                </form>
              )}
            </div>
