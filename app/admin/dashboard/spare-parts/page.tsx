...
import { getSpareParts, upsertSparePart, deleteSparePart, uploadSparePartImage, type SparePart } from '@/lib/api/spare-parts';
import AdminBadge from '@/components/admin/AdminBadge';
import AdminModal from '@/components/admin/AdminModal';
import ConfirmDialog from '@/components/admin/ConfirmDialog';
import AdminToast from '@/components/admin/AdminToast';

const CATS = ['DRIVE TRAIN', 'BRAKE PARTS', 'TYRE-TUBES', 'CHAINS', 'GEAR PARTS', 'HANDLE- STEM'];

type EditState = { id?: string; name: string; sku: string; category: string; price: number; stock_qty: number; compatibility: string; status: 'published' | 'draft'; image: string; description: string };
const empty: EditState = { name: '', sku: '', category: 'Brakes', price: 0, stock_qty: 0, compatibility: '', status: 'draft', image: '', description: '' };
...
export default function SparePartsPage() {
  ...
  async function handleImageFile(file: File) {
    setUploading(true);
    const { url, error } = await uploadSparePartImage(file);
    setUploading(false);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const { error } = await upsertSparePart({ name: edit.name, sku: edit.sku, category: edit.category, price: edit.price, stock_qty: edit.stock_qty, compatibility: edit.compatibility, status: edit.status, image: edit.image, description: edit.description }, edit.id);
    setSaving(false);
    if (error) { setToast({ msg: error.message, type: 'error' }); return; }
    setToast({ msg: edit.id ? 'Part updated.' : 'Part added.', type: 'success' });
    setModal(false);
    load();
  }
  ...
  async function handleDelete() {
    if (!deleteId) return;
    setDeleting(true);
    const { error } = await deleteSparePart(deleteId);
    setDeleting(false);
    setDeleteId(null);
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-semibold text-zinc-900">Spare Parts</h2>
          <p className="text-sm text-zinc-400 mt-0.5">{items.length} parts</p>
        </div>
      ...
      <input type="text" placeholder="Search parts…" value={search} onChange={(e) => setSearch(e.target.value)} className="admin-input w-64 mb-5" />
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50">
                {['Part Name', 'SKU', 'Category', 'Price', 'Stock', 'Compatibility', 'Status', 'Actions'].map((h) => (
            ...
            <tbody className="divide-y divide-zinc-50">
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-12 text-sm text-zinc-400">No spare parts found.</td></tr>
              ) : filtered.map((p) => (
                <tr key={p.id} className="hover:bg-zinc-50/50">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      {p.image ? (
                        <img src={p.image} className="w-9 h-9 object-cover bg-zinc-100 rounded" alt="" />
                      ) : (
                  ...
                  <td className="px-5 py-3.5 text-xs font-mono text-zinc-400">{p.sku}</td>
                  <td className="px-5 py-3.5 text-sm text-zinc-600">{p.category}</td>
                  <td className="px-5 py-3.5 text-sm font-semibold text-zinc-900">₹{p.price.toLocaleString('en-IN')}</td>
                  <td className="px-5 py-3.5 text-sm text-zinc-600">{p.stock_qty}</td>
                  <td className="px-5 py-3.5 text-xs text-zinc-400">{p.compatibility || '—'}</td>
                  <td className="px-5 py-3.5"><AdminBadge variant={p.status === 'published' ? 'green' : 'zinc'}>{p.status}</AdminBadge></td>
                  <td className="px-5 py-3.5">
                    <div className="flex gap-1">
                      <button onClick={() => { setEdit({ id: p.id, name: p.name, sku: p.sku, category: p.category, price: p.price, stock_qty: p.stock_qty, compatibility: p.compatibility, status: p.status, image: p.image, description: p.description }); setModal(true); }} className="w-7 h-7 flex items-center justify-center text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                      <button onClick={() => setDeleteId(p.id)} className="w-7 h-7 flex items-center justify-center text-zinc-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <AdminModal open={modal} onClose={() => setModal(false)} title={edit.id ? 'Edit Part' : 'Add Spare Part'} size="md">
        <div className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div><label className="admin-label">Part Name *</label><input className="admin-input" value={edit.name} onChange={(e) => setEdit((p) => ({ ...p, name: e.target.value }))} /></div>
            ...
            <div><label className="admin-label">Compatibility</label><input className="admin-input" value={edit.compatibility} onChange={(e) => setEdit((p) => ({ ...p, compatibility: e.target.value }))} placeholder="e.g. All Models, Road Only" /></div>
            <div>
      ...
      <ConfirmDialog open={!!deleteId} title="Delete Part" message="This will permanently delete the spare part." loading={deleting} onConfirm={handleDelete} onCancel={() => setDeleteId(null)} />
